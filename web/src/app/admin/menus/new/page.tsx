...
export default function NewMenuPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/menus', {
      ...
      body: JSON.stringify({ ...form, items, created_by: user?.id }),
    });
    ...
    else setError(result.error || 'Gagal membuat menu');
  }
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div>
        ...
        <p className="page-sub">Siapkan menu & item untuk customer</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <section className="card p-5 space-y-4">
          ...
          <div>
            ...
            <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="field" placeholder="Menu Senin, 27 Juli 2026" required />
          </div>
          <div>
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={loading} className="btn btn-primary">{loading ? 'Menyimpan...' : 'Simpan menu'}</button>
